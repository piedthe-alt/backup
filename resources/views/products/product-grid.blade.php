            ...
            <div class="product-card shadow-sm">
                ...
                <div class="card-body p-3 d-flex flex-column">
                    ...
                    <div class="price-badge" onclick="event.stopPropagation()" data-bs-toggle="modal"
                        data-bs-target="#productModal{{ $loop->index }}" data-product-id="{{ $product->id }}"
                        style="cursor: pointer;">
                        Rp {{ number_format($product->salesprice1, 0, ',', '.') }}
                    </div>

                    <!-- INFO HORIZONTAL -->
                    <div class="product-info" onclick="event.stopPropagation()" data-bs-toggle="modal"
                        data-bs-target="#productModal{{ $loop->index }}" data-product-id="{{ $product->id }}"
                        style="cursor: pointer;">
                        ...
                        <div class="info-item">
                            ...
                            <div class="stock-info-modern">
                                <div class="stock-current">
                                    <span>Sisa: <strong
                                            class="stock-current-value">{{ number_format($product->stock, 0, ',', '.') }}</strong></span>
                                </div>
                                <span
                                    class="stock-status-badge status-{{ strtolower(str_replace('_', '-', $stockStatus['status'])) }}"
                    ...
                    <div class="quantity-selector" onclick="event.stopPropagation();">
                        ...
                        <input type="number" class="qty-input" value="1" min="1"
                            max="{{ $product->stock }}" onchange="validateQty(this, {{ $product->stock }})">
